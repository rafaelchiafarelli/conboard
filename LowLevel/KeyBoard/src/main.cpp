// conKeyB -- the keyboard (evdev) input handler process. A thin main over the
// shared EvdevDevice (same engine as conJoyS / conMouse), matching keyboard
// input rules. The launcher spawns it with the profile and the USB port (-d).
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        cout << "usage: ./conKeyB -x \"/path/file.json\" [-d <usb-devpath>] [-p /dev/input/eventN]" << endl;
        return -1;
    }
    ...
    return condev::runDevice([&]{
        return new EvdevDevice(keyboard, "keyboard", jsonFileName, devNode, usbDevpath);
    });
}
