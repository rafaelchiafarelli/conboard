...
{
    io_connected = coms_socket.connect(coms_address);
    DevName = devName;
    io_address = coms_address;
    un_address = unique_number_address;
    local_connect();
}
...
void zmq_coms::local_connect(){
    ...
    hb_connected = st_socket.connect(un_address);
...
std::vector<std::string> zmq_coms::heartbeat()
{
    ...
    else
    {
        if(!io_connected)
        {
            io_connected = coms_socket.connect(io_address);
        }
        else
        {
