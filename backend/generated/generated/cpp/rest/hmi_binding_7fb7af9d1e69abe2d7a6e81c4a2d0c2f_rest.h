...
#ifndef HARPIA_REST_HMI_BINDING_7fb7af9d1e69abe2d7a6e81c4a2d0c2f
#define HARPIA_REST_HMI_BINDING_7fb7af9d1e69abe2d7a6e81c4a2d0c2f
...
// RESTful CRUD for hmi_binding, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/hmi_binding        list   (?limit=&offset= to paginate)
//   GET    <base>/hmi_binding/:id    read
//   POST   <base>/hmi_binding        create   (JSON or XML body)
//   PUT    <base>/hmi_binding/:id    update   (JSON or XML body)
//   DELETE <base>/hmi_binding/:id    delete
//
// GET list is paginated when the request supplies ?limit= (>0), or when
// hmi_binding declares a "pagination[size]" field (its size becomes the default
...
// the request must carry X-User: hmi_binding and X-Pswd: 7fb7af9d1e69abe2d7a6e81c4a2d0c2f, or it is rejected
// with HTTP 401 before the operation runs.
namespace harpia {
namespace rest {

// True iff the request carries the correct credential for hmi_binding. Exposed so it
// can be unit-tested directly.
inline bool authorized_hmi_binding(const crow::request& req) {
    return req.get_header_value("X-User") == "hmi_binding" &&
           req.get_header_value("X-Pswd") == "7fb7af9d1e69abe2d7a6e81c4a2d0c2f";
}

// content negotiation: XML when asked for, JSON otherwise
inline bool wants_xml_hmi_binding(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_hmi_binding(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_hmi_binding(const crow::request& req, ::hmi_binding* msg) {
    return body_xml_hmi_binding(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_hmi_binding(const crow::request& req,
                             const ::hmi_binding& msg, crow::response& res) {
    if (wants_xml_hmi_binding(req)) {
...
inline void register_hmi_binding(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/hmi_binding";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authorized_hmi_binding(req)) { res.code = 401; res.end(); return; }
            ::harpia::db::hmi_binding_dao dao(*dbp);
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            ...
            serialize_hmi_binding(req, msg, res);
            res.end();
        });
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            ...
            ::harpia::db::hmi_binding_dao dao(*dbp);
            ::hmi_binding msg;
            if (!parse_hmi_binding(req, &msg)) { res.code = 400; res.end(); return; }
...
#endif  // HARPIA_REST_HMI_BINDING_7fb7af9d1e69abe2d7a6e81c4a2d0c2f
