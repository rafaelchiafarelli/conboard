...
#ifndef HARPIA_REST_MODE_b13f689a5b6f99919ddaf4d1cc7eb7ac
#define HARPIA_REST_MODE_b13f689a5b6f99919ddaf4d1cc7eb7ac
...
// RESTful CRUD for mode, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/mode        list   (?limit=&offset= to paginate)
//   GET    <base>/mode/:id    read
//   POST   <base>/mode        create   (JSON or XML body)
//   PUT    <base>/mode/:id    update   (JSON or XML body)
//   DELETE <base>/mode/:id    delete
//
// GET list is paginated when the request supplies ?limit= (>0), or when
// mode declares a "pagination[size]" field (its size becomes the default
...
// the request must carry X-User: mode and X-Pswd: b13f689a5b6f99919ddaf4d1cc7eb7ac, or it is rejected
// with HTTP 401 before the operation runs.
namespace harpia {
namespace rest {

// True iff the request carries the correct credential for mode. Exposed so it
// can be unit-tested directly.
inline bool authorized_mode(const crow::request& req) {
    return req.get_header_value("X-User") == "mode" &&
           req.get_header_value("X-Pswd") == "b13f689a5b6f99919ddaf4d1cc7eb7ac";
}

// content negotiation: XML when asked for, JSON otherwise
inline bool wants_xml_mode(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_mode(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_mode(const crow::request& req, ::mode* msg) {
    return body_xml_mode(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_mode(const crow::request& req,
                             const ::mode& msg, crow::response& res) {
    if (wants_xml_mode(req)) {
...
inline void register_mode(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/mode";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authorized_mode(req)) { res.code = 401; res.end(); return; }
            ::harpia::db::mode_dao dao(*dbp);
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            ...
            ::harpia::db::mode_dao dao(*dbp);
            ::mode msg;
            if (!parse_mode(req, &msg)) { res.code = 400; res.end(); return; }
...
#endif  // HARPIA_REST_MODE_b13f689a5b6f99919ddaf4d1cc7eb7ac
