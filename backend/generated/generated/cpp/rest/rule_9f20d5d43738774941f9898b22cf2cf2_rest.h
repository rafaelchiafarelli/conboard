...
#ifndef HARPIA_REST_RULE_9f20d5d43738774941f9898b22cf2cf2
#define HARPIA_REST_RULE_9f20d5d43738774941f9898b22cf2cf2
...
// RESTful CRUD for rule, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/rule        list   (?limit=&offset= to paginate)
//   GET    <base>/rule/:id    read
//   POST   <base>/rule        create   (JSON or XML body)
//   PUT    <base>/rule/:id    update   (JSON or XML body)
//   DELETE <base>/rule/:id    delete
//
// GET list is paginated when the request supplies ?limit= (>0), or when
// rule declares a "pagination[size]" field (its size becomes the default
...
// the request must carry X-User: rule and X-Pswd: 9f20d5d43738774941f9898b22cf2cf2, or it is rejected
// with HTTP 401 before the operation runs.
namespace harpia {
namespace rest {

// True iff the request carries the correct credential for rule. Exposed so it
// can be unit-tested directly.
inline bool authorized_rule(const crow::request& req) {
    return req.get_header_value("X-User") == "rule" &&
           req.get_header_value("X-Pswd") == "9f20d5d43738774941f9898b22cf2cf2";
}

// content negotiation: XML when asked for, JSON otherwise
inline bool wants_xml_rule(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_rule(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_rule(const crow::request& req, ::rule* msg) {
    return body_xml_rule(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_rule(const crow::request& req,
                             const ::rule& msg, crow::response& res) {
    if (wants_xml_rule(req)) {
...
inline void register_rule(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/rule";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authorized_rule(req)) { res.code = 401; res.end(); return; }
            ::harpia::db::rule_dao dao(*dbp);
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            ...
            ::harpia::db::rule_dao dao(*dbp);
            ::rule msg;
            if (!parse_rule(req, &msg)) { res.code = 400; res.end(); return; }
...
#endif  // HARPIA_REST_RULE_9f20d5d43738774941f9898b22cf2cf2
