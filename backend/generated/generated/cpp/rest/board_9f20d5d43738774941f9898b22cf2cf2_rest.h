...
#ifndef HARPIA_REST_BOARD_9f20d5d43738774941f9898b22cf2cf2
#define HARPIA_REST_BOARD_9f20d5d43738774941f9898b22cf2cf2
...
// RESTful CRUD for board, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/board        list   (?limit=&offset= to paginate)
//   GET    <base>/board/:id    read
//   POST   <base>/board        create   (JSON or XML body)
//   PUT    <base>/board/:id    update   (JSON or XML body)
//   DELETE <base>/board/:id    delete
//
// GET list is paginated when the request supplies ?limit= (>0), or when
// board declares a "pagination[size]" field (its size becomes the default
...
// the request must carry X-User: board and X-Pswd: 9f20d5d43738774941f9898b22cf2cf2, or it is rejected
// with HTTP 401 before the operation runs.
namespace harpia {
namespace rest {

// True iff the request carries the correct credential for board. Exposed so it
// can be unit-tested directly.
inline bool authorized_board(const crow::request& req) {
    return req.get_header_value("X-User") == "board" &&
           req.get_header_value("X-Pswd") == "9f20d5d43738774941f9898b22cf2cf2";
}

// content negotiation: XML when asked for, JSON otherwise
inline bool wants_xml_board(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_board(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_board(const crow::request& req, ::board* msg) {
    return body_xml_board(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_board(const crow::request& req,
                             const ::board& msg, crow::response& res) {
    if (wants_xml_board(req)) {
...
inline void register_board(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/board";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authorized_board(req)) { res.code = 401; res.end(); return; }
            ::harpia::db::board_dao dao(*dbp);
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            ...
            ::harpia::db::board_dao dao(*dbp);
            ::board msg;
            if (!parse_board(req, &msg)) { res.code = 400; res.end(); return; }
...
#endif  // HARPIA_REST_BOARD_9f20d5d43738774941f9898b22cf2cf2
