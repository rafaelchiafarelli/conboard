...
#ifndef HARPIA_SOAP_TRIGGER_7fb7af9d1e69abe2d7a6e81c4a2d0c2f
#define HARPIA_SOAP_TRIGGER_7fb7af9d1e69abe2d7a6e81c4a2d0c2f
...
#include "xml/trigger_7fb7af9d1e69abe2d7a6e81c4a2d0c2f_xml.h"   // brings tinyxml2 + the XML runtime

// Minimal SOAP-over-HTTP access for trigger, backed by the CRUDL DAO. Register on
// a crow::SimpleApp with a base path; POST a SOAP envelope to <base>/trigger:
//   <soap:Body><get><id>N</id></get></soap:Body>          -> the trigger as XML
//   <soap:Body><set><trigger>...</trigger></set></soap:Body>       -> create
//   <soap:Body><update><trigger>...</trigger></update></soap:Body> -> update
...
//   <soap:Header><credentials><user>trigger</user><pswd>7fb7af9d1e69abe2d7a6e81c4a2d0c2f</pswd></credentials></soap:Header>
...
inline std::string envelope_trigger(const std::string& body) {
    return "<?xml version=\"1.0\"?>"
           "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
           "<soap:Body>" + body + "</soap:Body></soap:Envelope>";
}

// True iff the envelope carries the correct credential for trigger. The handler
// gates every operation on this; exposed so it can be unit-tested directly.
inline bool authorized_trigger(const ::tinyxml2::XMLDocument& doc) {
    ...
    return ::harpia::soap::detail::child_text(cred, "user") == "trigger" &&
           ::harpia::soap::detail::child_text(cred, "pswd") == "7fb7af9d1e69abe2d7a6e81c4a2d0c2f";
}

inline bool authorized_trigger(const std::string& soap_xml) {
    ::tinyxml2::XMLDocument doc;
    if (doc.Parse(soap_xml.c_str()) != ::tinyxml2::XML_SUCCESS) return false;
    return authorized_trigger(doc);
}

inline void register_trigger_soap(crow::SimpleApp& app, ::soci::session& db,
    ...
    ::soci::session* dbp = &db;
    app.route_dynamic(base + "/trigger").methods(crow::HTTPMethod::POST)(
        ...
        if (!authorized_trigger(doc)) {
            res.code = 401;
            reply(envelope_trigger(
        ...
        if (name == "get") {
            ...
            const auto* m = op->FirstChildElement();          // the trigger element
            ::trigger msg;
...
#endif  // HARPIA_SOAP_TRIGGER_7fb7af9d1e69abe2d7a6e81c4a2d0c2f
