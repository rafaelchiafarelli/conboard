...
#ifndef HARPIA_SOAP_RULE_5a67e5f27cce34a1ec5ac267a70f5d87
#define HARPIA_SOAP_RULE_5a67e5f27cce34a1ec5ac267a70f5d87
...
#include "xml/rule_5a67e5f27cce34a1ec5ac267a70f5d87_xml.h"   // brings tinyxml2 + the XML runtime

// Minimal SOAP-over-HTTP access for rule, backed by the CRUDL DAO. Register on
// a crow::SimpleApp with a base path; POST a SOAP envelope to <base>/rule:
//   <soap:Body><get><id>N</id></get></soap:Body>          -> the rule as XML
//   <soap:Body><set><rule>...</rule></set></soap:Body>       -> create
//   <soap:Body><update><rule>...</rule></update></soap:Body> -> update
...
//   <soap:Header><credentials><user>rule</user><pswd>5a67e5f27cce34a1ec5ac267a70f5d87</pswd></credentials></soap:Header>
...
inline std::string envelope_rule(const std::string& body) {
    return "<?xml version=\"1.0\"?>"
           "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
           "<soap:Body>" + body + "</soap:Body></soap:Envelope>";
}

// True iff the envelope carries the correct credential for rule. The handler
// gates every operation on this; exposed so it can be unit-tested directly.
inline bool authorized_rule(const ::tinyxml2::XMLDocument& doc) {
    ...
    return ::harpia::soap::detail::child_text(cred, "user") == "rule" &&
           ::harpia::soap::detail::child_text(cred, "pswd") == "5a67e5f27cce34a1ec5ac267a70f5d87";
}

inline bool authorized_rule(const std::string& soap_xml) {
    ::tinyxml2::XMLDocument doc;
    if (doc.Parse(soap_xml.c_str()) != ::tinyxml2::XML_SUCCESS) return false;
    return authorized_rule(doc);
}

inline void register_rule_soap(crow::SimpleApp& app, ::soci::session& db,
    ...
    ::soci::session* dbp = &db;
    app.route_dynamic(base + "/rule").methods(crow::HTTPMethod::POST)(
        ...
        if (name == "get") {
            ...
            const auto* m = op->FirstChildElement();          // the rule element
            ::rule msg;
...
#endif  // HARPIA_SOAP_RULE_5a67e5f27cce34a1ec5ac267a70f5d87
