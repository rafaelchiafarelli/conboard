...
#ifndef HARPIA_SOAP_MODE_5a67e5f27cce34a1ec5ac267a70f5d87
#define HARPIA_SOAP_MODE_5a67e5f27cce34a1ec5ac267a70f5d87
...
#include "xml/mode_5a67e5f27cce34a1ec5ac267a70f5d87_xml.h"   // brings tinyxml2 + the XML runtime

// Minimal SOAP-over-HTTP access for mode, backed by the CRUDL DAO. Register on
// a crow::SimpleApp with a base path; POST a SOAP envelope to <base>/mode:
//   <soap:Body><get><id>N</id></get></soap:Body>          -> the mode as XML
//   <soap:Body><set><mode>...</mode></set></soap:Body>       -> create
//   <soap:Body><update><mode>...</mode></update></soap:Body> -> update
...
//   <soap:Header><credentials><user>mode</user><pswd>5a67e5f27cce34a1ec5ac267a70f5d87</pswd></credentials></soap:Header>
...
inline std::string envelope_mode(const std::string& body) {
    return "<?xml version=\"1.0\"?>"
           "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
           "<soap:Body>" + body + "</soap:Body></soap:Envelope>";
}

// True iff the envelope carries the correct credential for mode. The handler
// gates every operation on this; exposed so it can be unit-tested directly.
inline bool authorized_mode(const ::tinyxml2::XMLDocument& doc) {
    ...
    return ::harpia::soap::detail::child_text(cred, "user") == "mode" &&
           ::harpia::soap::detail::child_text(cred, "pswd") == "5a67e5f27cce34a1ec5ac267a70f5d87";
}

inline bool authorized_mode(const std::string& soap_xml) {
    ::tinyxml2::XMLDocument doc;
    if (doc.Parse(soap_xml.c_str()) != ::tinyxml2::XML_SUCCESS) return false;
    return authorized_mode(doc);
}

inline void register_mode_soap(crow::SimpleApp& app, ::soci::session& db,
    ...
    ::soci::session* dbp = &db;
    app.route_dynamic(base + "/mode").methods(crow::HTTPMethod::POST)(
        ...
        if (name == "get") {
            ...
            const auto* m = op->FirstChildElement();          // the mode element
            ::mode msg;
...
#endif  // HARPIA_SOAP_MODE_5a67e5f27cce34a1ec5ac267a70f5d87
