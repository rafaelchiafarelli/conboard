...
#ifndef HARPIA_ZMQ_HMI_BINDING_5a67e5f27cce34a1ec5ac267a70f5d87
#define HARPIA_ZMQ_HMI_BINDING_5a67e5f27cce34a1ec5ac267a70f5d87
...
// with no client certs). Guarded separately from the per-message HARPIA_ZMQ_HMI_BINDING_5a67e5f27cce34a1ec5ac267a70f5d87 so
...
// push/pull: hmi_binding_sender pushes (stamping origin), hmi_binding_receiver pulls.
class hmi_binding_sender {
public:
    hmi_binding_sender(::zmq::context_t& ctx, const std::string& endpoint)
        : hmi_binding_sender(ctx, endpoint, origin_id()) {}
    ...
    static const std::string& origin_id() {
        static const std::string id = "505244579954278847";
        return id;
    }
    const std::string& origin() const { return origin_; }
    bool send(const ::hmi_binding& msg) {
        ::hmi_binding stamped = msg;
...
class hmi_binding_receiver {
public:
    // Trailing curve keys default to disabled (empty), so callers who don't
    // pass any get today's plaintext behavior unchanged.
    hmi_binding_receiver(::zmq::context_t& ctx, const std::string& endpoint,
...
// pub/sub (streaming/event): hmi_binding_publisher publishes (stamping origin), hmi_binding_subscriber receives.
class hmi_binding_publisher {
public:
    hmi_binding_publisher(::zmq::context_t& ctx, const std::string& endpoint)
        : hmi_binding_publisher(ctx, endpoint, origin_id()) {}
    // explicit-origin constructor: pass any externally-sourced id (e.g. if a
    // future broker hands one out); the default constructor above already
    // supplies a runtime-unique id for many-to-* senders on its own. The
    // trailing curve keys default to disabled (empty), so callers who don't
    ...
    const std::string& origin() const { return origin_; }
    bool publish(const ::hmi_binding& msg) {
        ::hmi_binding stamped = msg;
        stamped.set_originator_5a67e5f27cce34a1ec5ac267a70f5d87(origin_);
        std::string bytes;
        if (!stamped.SerializeToString(&bytes)) return false;
...
class hmi_binding_subscriber {
public:
    // Trailing curve keys default to disabled (empty), so callers who don't
    // pass any get today's plaintext behavior unchanged.
    hmi_binding_subscriber(::zmq::context_t& ctx, const std::string& endpoint,
...
#endif  // HARPIA_ZMQ_HMI_BINDING_5a67e5f27cce34a1ec5ac267a70f5d87
